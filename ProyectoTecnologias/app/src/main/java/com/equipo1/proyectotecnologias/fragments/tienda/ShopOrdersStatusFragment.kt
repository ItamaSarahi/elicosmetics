...
class ShopOrdersStatusFragment : Fragment() {
    ...
    var adapter: OrdenesTiendaAdapter? = null
    ...
    override fun onCreateView(
        ...
        myView = inflater.inflate(R.layout.fragment_tienda_status_ordenes, container, false)
        ...
        ordersProvider?.getOrdersByStatus(status)?.enqueue(object: Callback<ArrayList<Order>> {
            override fun onResponse(call: Call<ArrayList<Order>>, response: Response<ArrayList<Order>>) {
                if (response.body() != null) {
                    ...
                    adapter = OrdenesTiendaAdapter(requireActivity(), orders!!)
